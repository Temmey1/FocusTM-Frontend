...
export default function CheckoutSuccessPage() {
  ...
  return (
    <div className="max-w-lg mx-auto px-8 pt-40 pb-28 text-center">
      ...
      <h1 className="font-display font-light text-[clamp(28px,4vw,44px)] mb-4">
        Thank you{name ? `, ${name.split(" ")[0]}` : ""}
      </h1>
      <p className="text-[13px] text-ftm-muted leading-[1.85] mb-2">
        Your order <span className="text-ftm-white font-mono">#{order}</span>{" "}
        has been received.
      </p>
      {total > 0 && (
        <p className="font-display text-[22px] mb-8">{formatNaira(total)}</p>
      )}

      {method === "whatsapp" ? (
        <p className="text-[12px] text-ftm-muted leading-[1.8] mb-8">
          Tap below to confirm your order and complete payment directly with our
          team on WhatsApp.
        </p>
      ) : (
        <p className="text-[12px] text-ftm-muted leading-[1.8] mb-8">
          We&apos;ve received your order and will begin processing once payment
          is confirmed.
        </p>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        {method === "whatsapp" && (
          <a
            href={buildWhatsAppMessage(order, total, name)}
            target="_blank"
            rel="noreferrer"
      ...
      <Link
        href="/shop"
        className="block mt-10 text-[11px] text-ftm-dim hover:text-ftm-muted transition-colors underline-slide"
      >
        Continue Shopping
      </Link>
    </div>
