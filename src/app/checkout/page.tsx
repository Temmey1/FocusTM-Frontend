...
const inputCls =
  "w-full bg-transparent border border-ftm-line px-4 py-3 text-[12px] text-ftm-white placeholder:text-ftm-dim focus:border-ftm-offwhite outline-none transition-colors";
const selectCls =
  "w-full bg-ftm-black border border-ftm-line px-4 py-3 text-[12px] text-ftm-white focus:border-ftm-offwhite outline-none transition-colors appearance-none cursor-pointer";
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    Promise.all([getDeliveryLocations(), getPickupLocations()]).then(
      ([d, p]) => {
  ...
  const activeDeliveryLoc = deliveryLocations.find(
    (l) => l.state === selectedState,
  );
  const activePickupLoc = pickupLocations.find(
    (l) => l.id === selectedPickupId,
  );
  const deliveryFee =
    method === "delivery" ? (activeDeliveryLoc?.fee ?? 2500) : 0;
  ...
  const handleSubmit = async () => {
    ...
    if (method === "delivery" && (!selectedState || !address)) {
      toast.error("Please select a state and enter your address.");
      return;
    }
    if (method === "pickup" && !selectedPickupId) {
    ...
    try {
      ...
      router.push(
        `/checkout/success?order=${orderNumber}&method=${paymentMethod}&total=${total}&name=${encodeURIComponent(fullName)}`,
      );
    } catch {
  ...
  return (
    <div className="max-w-5xl mx-auto px-8 pt-36 pb-28 grid grid-cols-1 md:grid-cols-3 gap-14">
      ...
      >
        <h1 className="font-display font-light text-[clamp(32px,4.5vw,52px)]">
          Checkout
        </h1>

        <div>
          <p className="text-[9px] uppercase tracking-[0.35em] text-ftm-muted mb-5">
            Contact & Delivery
          </p>
        ...
        <div>
          <p className="text-[9px] uppercase tracking-[0.35em] text-ftm-muted mb-4">
            Fulfillment Method
          </p>
            ...
            {(["delivery", "pickup"] as DeliveryMethod[]).map((m) => (
              <button
                ...
                className={`flex-1 px-5 py-3 border text-[10px] uppercase tracking-[0.18em] transition-all duration-200 ${
                  method === m
                    ? "border-ftm-white text-ftm-white"
                    : "border-ftm-line text-ftm-muted hover:border-ftm-offwhite"
                }`}
              >
        ...
        {locationsLoading ? (
            ...
            <div className="relative">
              ...
              >
                {deliveryLocations.map((l) => (
                  <option key={l.id} value={l.state}>
                    {l.state} — {l.fee === 0 ? "Free" : formatNaira(l.fee)}
                  </option>
                ))}
              </select>
              <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-ftm-muted text-xs">
                ▾
              </span>
            </div>
            <input
          ...
          <p className="text-[12px] text-ftm-muted border border-ftm-line p-5">
            No pickup locations are configured yet — please choose Home Delivery
            instead.
          </p>
        ) : (
          <div className="flex flex-col gap-2">
            {pickupLocations.map((l) => (
              <button
                key={l.id}
                onClick={() => setSelectedPickupId(l.id)}
              ...
              >
                <p className="text-[11px] uppercase tracking-[0.15em] text-ftm-white">
                  {l.state}
                  {l.city ? `, ${l.city}` : ""}
                </p>
        ...
        <div>
          ...
          <div className="flex flex-col gap-3">
            {(
              [
                {
                  ...
                  sub: "Confirm and pay directly with our team.",
                },
                {
                  id: "monnify",
              ...
              >
                <p
                  className={`text-[10px] uppercase tracking-[0.18em] ${paymentMethod === opt.id ? "text-ftm-white" : "text-ftm-muted"}`}
                >
                  {opt.title}
                </p>
      ...
      <motion.div
        ...
        className="border border-ftm-line p-7 h-fit sticky top-28"
      >
        <h2 className="font-display font-light text-[22px] mb-7">
          Order Summary
        </h2>
        <div className="flex flex-col gap-3 mb-6">
          {items.map((item) => (
            <div
              ...
              className="flex justify-between text-[11px] text-ftm-muted"
            >
        ...
        <div className="flex justify-between mb-8">
          <span className="text-[11px] uppercase tracking-[0.15em] text-ftm-muted">
            Total
          </span>
