...
export default function TrackOrderPage({
  ...
  useEffect(() => {
    api
      .get(`/orders/track/${params.orderNumber}`)
  ...
  if (error || !order) {
    return (
      <div className="max-w-md mx-auto px-8 pt-40 pb-28 text-center">
        ...
        <p className="text-[13px] text-ftm-muted">
  ...
  return (
    <div className="max-w-2xl mx-auto px-8 pt-36 pb-28">
      ...
      <h1 className="font-display font-light text-[clamp(26px,4vw,40px)] mb-1 font-mono">
        #{order.orderNumber}
      </h1>
      <p className="text-[12px] text-ftm-muted mb-10">
        Placed{" "}
      ...
      {cancelled ? (
        <div className="border border-red-900/40 bg-red-950/10 p-5 mb-10">
          <p className="text-[12px] uppercase tracking-[0.15em] text-red-400">
            Order Cancelled
          </p>
        </div>
      ) : (
        <div className="flex items-center mb-12">
          {STEPS.map((step, i) => (
            <div key={step} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center">
                <div
                  className={`h-7 w-7 rounded-full flex items-center justify-center border transition-colors ${
                    i <= stepIndex
                      ? "bg-ftm-white border-ftm-white text-ftm-black"
                      : "border-ftm-line text-ftm-dim"
                  }`}
                >
                  {i <= stepIndex ? (
                ...
                <span
                  className={`text-[8px] uppercase tracking-[0.12em] mt-2 capitalize ${i <= stepIndex ? "text-ftm-white" : "text-ftm-dim"}`}
                >
                  {step}
                </span>
              </div>
              {i < STEPS.length - 1 && (
                <div
                  className={`flex-1 h-px mx-2 mb-5 ${i < stepIndex ? "bg-ftm-white" : "bg-ftm-line"}`}
                />
              )}
            </div>
          ))}
      ...
      <div className="border border-ftm-line p-6 mb-6">
        ...
        <div className="flex justify-between text-[12px] text-ftm-muted mb-1">
          <span>Subtotal</span>
          <span>{formatNaira(order.subtotal)}</span>
        </div>
        <div className="flex justify-between text-[12px] text-ftm-muted mb-3">
          <span>Delivery</span>
          <span>{formatNaira(order.deliveryFee)}</span>
        </div>
        <div className="flex justify-between font-display text-[18px]">
          <span>Total</span>
